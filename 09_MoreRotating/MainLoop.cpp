...
void MainLoop::loop()
{
	...
	RAIIArray<> vertexBuffer(glDeleteBuffers, glGenBuffers);
	RAIIArray<> elementBuffer(glDeleteBuffers, glGenBuffers);
	RAIIArray<> rotaryAO(glDeleteVertexArrays, glGenVertexArrays);
	RAIIArray<> texture(glDeleteTextures, glGenTextures);
	RAIIArray<> secondTexture(glDeleteTextures, glGenTextures);
	float rotation = 0;
	{
		// fill vertices
		float vertices[] = {
