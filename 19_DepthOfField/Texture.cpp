...
Texture Texture::fromSurface(SDL_Surface* surf, bool flip_texture, GLenum wrap_s, GLenum wrap_t, GLenum min_filter, GLenum mag_filter)
{
	// create texture
	RAIIArray<> texture(glDeleteTextures, glGenTextures);
	// flip image
	void* data = surf->pixels;
	std::vector<char> flipped_data;
	if (flip_texture) {
		flipped_data.resize(surf->h * surf->pitch);
...
Texture Texture::fromFile(std::string filepath, bool flip_texture, GLenum wrap_s, GLenum wrap_t, GLenum min_filter, GLenum mag_filter)
{
	...
	Texture t = Texture::fromSurface(imgSurface, flip_texture, wrap_s, wrap_t, min_filter, mag_filter);
